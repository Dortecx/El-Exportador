...
import { matchTrack, simulateOfflineMatch } from "./matcher.js";
...
export async function run(filePath, options) {
    ...
    for (let i = 0; i < tracks.length; i++) {
        ...
        if (options.verbose) {
            console.log(`${statusIcon} [${i + 1}/${tracks.length}] ${track.artist ? `${track.artist} - ` : ""}${track.title} ${confidence}`);
            if (result.bestMatch) {
...
async function runDryRun(tracks, threshold, verbose, offline) {
    ...
    if (YOUTUBE_API_KEY) {
        ...
        if (!isValid) {
            throw new Error("YOUTUBE_API_KEY environment variable required for dry-run mode without OAuth2.\n" +
